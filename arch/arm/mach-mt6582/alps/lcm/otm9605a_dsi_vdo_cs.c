...
static struct LCM_setting_table lcm_initialization_setting[] = {
	...
	*/
	...
	{0xD9,	1,	{0x64}},
	{0x00,	1,	{0x00}},
	{0xE1,	16,	{0x03,0x11,0x17,0x0f,0x08,0x11,0x0b,0x0a,0x03,0x07,0x0b,0x06,0x0d,0x0f,0x0b,0x0A}},
	{0x00,	1,	{0x00}},
	{0xE2,	16,	{0x03,0x11,0x18,0x0e,0x07,0x11,0x0b,0x0a,0x03,0x06,0x0a,0x07,0x0e,0x0f,0x0c,0x0A}},	
	{0x00,	1,	{0xB1}},
	{0xC5,	1,	{0x28}},
	{0x00,	1,	{0x80}},
	{0xC4,	1,	{0x9C}},
...
*/
    {0x22, 1, {0x00}}, 
    {REGFLAG_DELAY, 40, {}}, // All pixel off 
        
    {0x10, 1, {0x00}}, 
    {REGFLAG_DELAY, 50, {}},  // Sleep in 
    
    {0x28, 1, {0x00}}, 
    {REGFLAG_DELAY, 100, {}},  // Sleep in 
...
static unsigned int lcm_compare_id(void)
{
		...
		#if defined(BUILD_LK)
			printf("otm9608a_qhd uboot %s \n", __func__);
			printf("%s id = 0x%x: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", __func__, id,buffer[0],buffer[1],buffer[2],buffer[3],buffer[4]);
		#else
			printk("otm9608a_qhd kernel %s \n", __func__);
			printk("%s id = 0x%x: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", __func__, id,buffer[0],buffer[1],buffer[2],buffer[3],buffer[4]);
		#endif
...
#else
static unsigned int lcm_compare_id_9605a(void)
{
...
static unsigned int lcm_compare_id(void)
{
	...
	int result = 0;
	int lcmid;
	if(lcm_compare_id_9605a())
		{
	IMM_GetOneChannelValue(1,adcdata,&lcmid);
	#if defined(BUILD_LK)
	printf("OTM9605A cs channelValue=%d\n",lcmid);
	#else 
	printk("OTM9605A cs channelValue=%d\n",lcmid);
	#endif

	lcmid = lcmid * 1500/4096; 
	#if defined(BUILD_LK)
			...
			printk("%s lcmid = %d \n", __func__, lcmid);
	#endif
	if(lcmid < 200) //dijing 1.4V,cs 0v
	{
			return 1;
	}
	else
